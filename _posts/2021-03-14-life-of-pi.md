...
Believe it or not, I actually manually typed the above digits by heart. Back in the days, I used to be able to recite far more digits, probably close to 100 even. Unfortunately I don't have Sheldon Cooper's eidetic memory, so that's how much stayed in my memories as I grew older. I did a quick lookup online, that's actually a correct recollection! Woohoo!
...
That's so profound. You can actually find your social insurance number, date of birth, in fact, any finite sequence of digits in `pi`'s infinite digits, as long as you search further enough. I've found a [website](http://mypiday.com/) that visualizes searches for birthdays in `pi` for you.
...
On that note, I'd like to ramble a bit about the nature of `pi`, or more on how little we human race know about it really. So `pi` is what we call, an `irrational number`. To understand it, we'd have to know what a rational number is first.  
...
It was dated back to ancient Greece. Pythagoras, who is best known for The Pythagorean Theorem[^fn1] in the western world, had claimed all numbers in life can be represented by ratios of integers, i.e. are all `rational numbers` as explained in last section.
...
`pi`, just like square root of 2 here, is an irrational number than cannot be converted to a ratio by integers.
...
As an engineer, I'm oftentimes amazed yet also baffled by the fact that in our real world, we have constants, such as `pi` and speed of light (think this Einstein guy said we can't travel faster than it, isn't that disturbing?), etc. That can be a discussion easily leading to religions, or a higher power / designer of the world we're living in. Cuz programmers would know that, when we're feeling lazy, we may just hardcode stuff (or better yet, mark it as a `TODO` too, so it'll stay there until end of time. :D), or how often we just declare a constant:
