...
> PCA is a statistical procedure that extracts the most important features of a dataset, by reducing the dimensionality of high-dimensional datasets while preserving the original structure and relationships inherent to the original dataset. It'd fall under the unsupervised learning category.
...
Lots of people go further than that, like when reading an end-user license agreement (EULA) and other fine prints, most people would spend no more than 10 seconds before clicking the big "I Agree" button. We probably should read them though[^fn4]. :D
