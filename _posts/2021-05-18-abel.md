...
Now we have another mechanism to calculate private and public key pairs! The actual implementation of ECDSA signing algorithm is a bit more complicated, as there're more steps like putting the curve on a finite field, etc. But that's pretty much the gist of the idea. There're many upsides of using ECDSA: despite its abstract calculations, it provides shorter keys and faster speeds for the level of security needed. But it's less adopted compared to RSA at the moment.
